...
| `UI/Game/EmueraConsole.Print.cs` | Text/style/buttons, HTML, images, shapes, backgrounds, window title and log buffer are observable. | Revisioned `PresentationSnapshot`; the delta wire type is reserved but not currently emitted. Frontend render objects are projections. |
