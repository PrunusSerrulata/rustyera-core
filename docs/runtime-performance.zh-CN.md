...
- TUI 使用持久化 stat/hash 源码索引。普通启动和重启只重新读取签名变化的 UTF-8 文件，先以
  ...
  脚本”仍执行完整内容扫描。
- 编译缓存 v3 将 artifact、增量状态和规范化项目快照流式编码为紧凑 MessagePack，再用 zstd
  level 3 压缩，同时校验压缩 section 及整体摘要。完整 eraTW 的 v3 产物为 124,534,530 bytes，
  相对同一测量链路的 v2 样本 143,674,679 bytes 缩小约 13.3%。前端在项目加载成功后先启动
  游戏，延迟 5 秒再通过 caller-pumped 导出请求启动后台编码，并以 250 ms 低频请求推进完成；
  因此最终专项样本在 14.20 秒到达日 1、19.47 秒完成缓存落盘，缓存生成不再位于冷启动关键路径。
  成功写入 v3 后会清理旧 v1/v2 文件。
- 同一 v3 缓存的下一次启动明确报告 `runtime.compiled_cache_hit`，不会重新分析或编译源码；本机
  到日 1 为 13.70 秒。当前大项目的解压、MessagePack 反序列化及不可信字节码完整验证仍接近
  冷编译成本，因此命中缓存并不等同于瞬时启动，后续优化应继续针对该可信边界内存流量。
