...
<a id="upstream-7b69"></a>

## 原版 upstream 7b69 跟进（2026-09-11）

固定原版语义 `7b69ebd27378c03c32b6477b74901bfc3d33223c`，wrapper
`c94bf1de2c4ecc0f876a913f0c8ec1035d3f06b4`；蛇版语义和行为保持不变。保留
`emuera.em` 方言名称，按现有兼容身份升级，不增加协议字段或旧原版方言。

| 批次 | 范围和规模 | 依赖与交付 |
|---|---|---|
| A | `.als` 首次定义保留；普通 `CHKDATA` 的 `RESULT:1`，局部小规模合并验证 | 原版 semantic/policy 1→2；两个功能分别提交 |
| B | 传统字符串 UTF-16 单元计数、往返编码及 CP932 回退、索引/截取/查找、常量求值，中等规模 | A 验收后实施，原版 2→3；独立提交，审计全部 LangManager 调用点 |
| C | TUI C ABI、Web WASM 与 Tauri 消费和绑定，跨仓库集成 | A/B 完成后统一绑定最终完整 core SHA，一次更新各仓依赖与锁 |

B 保持合法 UTF-8 字符串；UTF-16 截取产生的孤立代理项转为 U+FFFD，登记有意差异。
U 系列、UTF-8 源码输入和可移植 FORM 显示列宽不改变。策略必须按兼容身份选择，
共用计数实现供 analyzer 和 VM 使用，不能全局修改旧计数而影响蛇版。

旧字节码/缓存/snapshot 依完整身份拒绝或失效，不迁移旧 snapshot；传统游戏存档
继续既有游戏标识和版本检查。各批独立一次重构审查、一次全量及 60 分钟测试预算，
失败只定向复验；静态门禁通过后才运行双 oracle 和固定真实客户端场景。C 验证 TUI
...
<a id="snake-upstream-5717045"></a>

## 蛇版 Skiav13 跟进（2026-09-11）

参考语义 `57170459b3d5ca175a1c57933058b569088bee0e`，wrapper
`851c40c9bec73bd3819177d5f107ab8b6502a14a`；原版保留 `7b69ebd2` 与 policy 3。
复用原版已跟进能力，既有历史捕获不改写。蛇版 TW `667b9cd0` 静态扫描有大量
RAND/传统字符串调用和 4 处 CHKDATA，未见预设 ERD 与来源探针使用。用户明确将
预设 ERD 作为已有能力扩展纳入；EXIST_IN_CSV/EXIST_IN_ERD 与通用 Float 暂缓。

| 批次 | 实施与契约 | 依赖及验证 |
|---|---|---|
| A | 复用原版 UTF-16 编码往返/CP932 回退及普通 CHKDATA RESULT:1；snake 12→13 | 两功能合并审查测试、分别提交；四编码 provider 核对、双 oracle |
| B | 整数 RAND 变量/函数独立入口及钳制；每会话每入口一次结构化警告；snake 13→14 | A 后；保留解析期零/遗漏拒绝，钳制不推进 SFMT，动态 FORM 与快照复验 |
| C | CSV 后/反查前合并预设 ERD 名表与 ITEM 价格；snake 14→15 | B 后；CSV 优先、跨表重名、显式零价格、UseERD、缓存与用户 ERD 回归 |
| D | TUI、三浏览器 WASM、macOS Tauri 固定真实客户端消费 | A–C 完成后统一完整 core SHA、依赖和锁；旧缓存回退与旧快照拒绝 |

A 保持合法 UTF-8，孤立代理项转 U+FFFD；U 系列和 FORM 显示列宽不变。先核对
蛇版 .NET 8 实际 provider 的全部宽度与现有表，一致则复用，不一致则按 profile 分表。
B 复用已有调用遗漏元数据，不用 i64::MIN 冒充遗漏；源码首参数遗漏及变量超维仍拒绝，
嵌套下标显式加括号。只对实际执行的蛇版普通 RAND
钳制，不改原版、CompatiRAND、SFMT 算法或已登记随机差异。会话警告区别于上游进程
静态状态，使用 LogOnly 并恢复去重状态；VM snapshot 格式 20→21，旧格式拒绝。
C 复用现有 ERD 输入/名表/GETNUM/角色模板与缓存，不新增来源探针持久字段。
44 个预设文件 stem 映射到既有 29 个名表槽位；路径排序复用 VM 已有的固定 .NET
OrdinalCasing 和大小写表，下移到公共兼容层供 VM、CSV 共用，不复制映射或新增依赖。
每批测试前唯一重构审查、每套全量一次、60 分钟预算，失败仅定向复验；静态通过
后逐例执行/校验/差分。完成后才写最终实施记录。过程材料位于本任务忽略目录
`.audit/snake-upstream-5717045-20260911/`，不将过程状态写入实施日志。
不扩展发布版本、自主游玩、性能循环、额外平台或自动推送。
