...
| D16 | 音频“期望状态”控制 | `SOUNDCONTROL/BGMCONTROL` 的 pause/resume/stop/rate/preserve-pitch；固定蛇版 API 没有 Seek action | 改变 AudioState 与一次性 effect 顺序；客户端支持程度不同 | 批次 5 已实现 revision-bound 音频 effect 与实际 provider；10 个 sound channel 和 BGM，缺能力明确失败；不把实际播放进度当 core 状态 | M/L |
...
| D18 | 蛇版传统存档互操作 | 标准 Emuera 1808 Binary/GZip/Text 的 ordinary 与 GLOBAL 独立作用域 | 普通存档不恢复 GLOBAL、SFMT 或 SQL；RANDDATA 仅显式 INITRAND 生效 | 批次 5 已完成非 Float 标准 codec 和项目 sav 双向验证；无私有 envelope/legacy fallback，损坏/未知/Float 原子拒绝；TUI 真实 TW 因像素能力限制未验证 | L |
...
| C07 | `GETSOUNDORBGMINFO/ISPLAYING*` 直接查询播放器时长、进度和实际播放 | 播放器时钟、解码、后台节流和无音频客户端不一致 | 音乐补丁需要时长、进度和播放状态 | “期望状态”放 D16；批次 5 通过 revision-bound `audio_observation@1` 返回实际 duration/position/state 与单调时间戳。Web/Tauri 已验证；TUI 缺少 provider 时明确失败，不返回伪造 stopped | 5 |
