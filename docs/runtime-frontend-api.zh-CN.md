...
| `channel_version` | `Runtime` channel 当前为 `17.0`；`Debug` channel 当前为 `4.0`。 |
...
| 字段 | 含义 |
| --- | --- |
| `runtime_versions` | 前端接受的 runtime protocol 版本区间。当前应包含 `17.0`。 |
...
规范化展示状态。Protocol 17.0 会向当前 session 唯一的权威投影前端发送携带 presentation
