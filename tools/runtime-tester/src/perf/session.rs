...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
struct StableInputWait<'a> {
    kind: era_runtime_protocol::WaitKind,
    stability: era_runtime_protocol::WaitStability,
    one_input: bool,
    stop_message_skip: bool,
    system_input: bool,
    mouse_input: bool,
    default_value: Option<&'a era_runtime_protocol::ProtocolValue>,
    display_time: bool,
    timeout_message: Option<&'a str>,
    viewport_policy: era_runtime_protocol::InputViewportPolicy,
}

impl<'a> From<&'a InputWait> for StableInputWait<'a> {
    fn from(wait: &'a InputWait) -> Self {
        Self {
            kind: wait.kind,
            stability: wait.stability,
            one_input: wait.one_input,
            stop_message_skip: wait.stop_message_skip,
            system_input: wait.system_input,
            mouse_input: wait.mouse_input,
            default_value: wait.default_value.as_ref(),
            display_time: wait.display_time,
            timeout_message: wait.timeout_message.as_deref(),
            viewport_policy: wait.viewport_policy,
        }
    }
}
...
mod tests {
    ...
    fn sample_wait() -> InputWait {
        InputWait {
            ...
            submission_token: era_runtime_protocol::InteractionToken { epoch: 2, id: 8 },
            countdown_remaining_ms: Some(9),
    ...
    #[test]
    fn normalized_wait_omits_session_local_identity_and_deadlines() -> AuditResult<()> {
        let mut session = PerfSession::new();
        session.presentation.wait = Some(sample_wait());
        let baseline = session.normalized_state(&[], &BTreeMap::new())?;

        let wait = session.presentation.wait.as_mut().expect("sample wait");
        wait.wait_id = 99;
    ...
    #[test]
    fn presentation_text_joins_runs_within_each_logical_line() {
        let text_run = |text: &str, bold| DisplayRun::Text {
            text: text.into(),
            style: era_runtime_protocol::TextStyle {
                bold,
                ..era_runtime_protocol::TextStyle::default()
            },
            system_text: None,
        };
        let line = |line_id, runs| DisplayLine {
            line_id,
            temporary: false,
            logical_line_start: true,
            line_end: true,
            alignment: era_runtime_protocol::LineAlignment::Left,
            runs,
            text_background_eligible: true,
        };
        let mut session = PerfSession::new();
        session.presentation.lines = vec![
            line(1, vec![text_run("foo", false), text_run("bar", true)]),
            line(2, vec![text_run("tail", false)]),
        ];

        assert_eq!(session.presentation_text(), "foobar\ntail");
    }
}
