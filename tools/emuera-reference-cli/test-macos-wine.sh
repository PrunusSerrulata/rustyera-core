...
printf '%s\n' \
    ...
    '{"id":"wine-native-tail","op":"run","entry":"ORACLE_NATIVE","watch":["RESULT:0","RESULT:1","RESULT:2","RESULT:3","RESULT:4","RESULT:5","RESULT:6","RESULT:7","RESULT:8","RESULT:9","RESULT:10","RESULT:11","RESULTS:0","RESULTS:1","RESULTS:2","RESULTS:3","RESULTS:4","RESULTS:5","RESULTS:6","RESULTS:7"]}' \
...
jq -e -s '
    ...
    map(.id) == [
        ...
        "wine-run", "wine-native-tail", "wine-reflection", "wine-map", "wine-presentation", "wine-structured", "wine-input", "wine-reset"
    ] and
    all(.[]; .ok == true) and
    (map(select(.id == "wine-load"))[0].result.termination == "waitingInput") and
    (map(select(.id == "wine-project"))[0].result.functions | map(.name) | sort == ["ORACLE_INPUT", "ORACLE_MAP", "ORACLE_NATIVE", "ORACLE_PRESENTATION", "ORACLE_REFLECTION", "ORACLE_STRUCTURED", "ORACLE_TEST", "SYSTEM_TITLE"]) and
